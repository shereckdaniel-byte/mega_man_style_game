...
| **`backblaze.pixellab.ai` is not in the environment's allowed domains** | Blocks **stage 4's** tileset, and any tileset generated from here on | **Open. Re-probed a third time at M7d, including the MCP tool and four endpoints nobody had tried, all dead — see SPRITES.md §8g for the table.** The fix is a change to the environment's allowed domains and cannot be made from inside a session. `tools/fetch_tilesets.sh` is the whole recovery once it is: it holds the id table and checks the host first. **Scope corrected at M7d: exactly one tileset is stranded (Mirror Field's). Stages 5–8 and the fortress are greyboxed because no tileset has ever been generated for them, which this host does not block.** Older note follows. Tileset *metadata* is served from `api.pixellab.ai` and downloads fine; the spritesheet PNG 302s to `backblaze.pixellab.ai`, which the egress proxy answers 403 to. Every alternate route was re-probed at M6j and all still redirect. A generated tileset stays on PixelLab's server, so opening the host recovers it without paying again — SPRITES.md §8 has had this written down since stage 1 |
