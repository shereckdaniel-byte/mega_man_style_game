...
**Built at M6o.** `Pickup` is the capsule; weapon energy mirrors health at 2 and 10,
because a weapon bar is the same 28 ticks. Two rules that are not in the numbers:

- **A capsule that cannot be used is refused, not consumed.** Health at full health, weapon
  energy on the buster or on a full weapon — the capsule stays on the floor for when it is
  worth something. That is the decision `PauseMenu.use_etank` already made about spending
  a tank at full health, applied to the other end of the same transaction. An E-tank
  capsule is the exception and is taken at full health, because a tank is *banked* rather
  than spent.
- **Four silhouettes, not four colours.** Health is a square, weapon energy a flat bar, a
  life a disc, an E-tank a tall canister; size says small or large. Six items told apart
  only by hue would undo what `PhaseBlock` was careful about one system earlier.

**Where enemy stats live.** This document used to promise an `EnemyData` resource holding
hp, contact damage and a drop table. `resources/enemies/` is empty and always has been:
every per-enemy number is an `@export` on `Enemy`, and the drop table is one too. Building
a resource layer for one field would have been a second way to configure an enemy rather
than a better one.
